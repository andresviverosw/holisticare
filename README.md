...
4. [Especificación de la API](#4-especificacion-de-la-api)
...
Guía de despliegue: `docs/deploy-entrega2-demo.md` (blueprint y fixes en rama `feature-entrega2-AVW`).
...
**Alternativas existentes vs HolistiCare (posicionamiento):**

| Enfoque / herramienta | Continuidad entre sesiones | Personalización del plan | Medición de outcomes | Evidencia / RAG clínico | Cumplimiento NOM-024 (aprobación humana) |
|----------------------|----------------------------|--------------------------|----------------------|-------------------------|------------------------------------------|
| Papel / expediente físico | Baja | Baja | Baja | No | Manual, frágil |
| Excel / Google Sheets | Media | Baja | Media | No | No nativo |
| Doctoralia (agenda/citas) | Baja (enfocado en citas) | Baja | Baja | No | No diseñado para plan terapéutico |
| SimplePractice / Jane App | Alta (expediente general) | Media | Media | No / limitada | Depende de proceso clínico externo |
| **HolistiCare (MVP)** | **Alta (intake + plan + diario + sesiones)** | **Alta (plan multi-semana + memory bank)** | **Alta (tendencias, mesetas, trayectoria)** | **Sí (pgvector + citaciones REF-ID)** | **Sí (pending_review obligatorio)** |
...
7. **Analítica y predicción** — tendencias de outcomes, detección de mesetas y recomendaciones de ajuste (`US-PRED-001` / `US-PRED-002`).
8. **Memory bank** — biblioteca de planes aprobados reutilizables como plantillas (US-PLAN-004).
9. **Exportación PDF** — planes aprobados exportables para revisión y archivo clínico.
...
3. **Plan Review** — revisión del borrador generado, citaciones, aprobar/rechazar/editar, export PDF.
...
Guía de uso clínico: `docs/07-user-guide.md`. Checklist demo: `docs/demo-smoke-checklist.md`.

Capturas / video: enlazar grabación del flujo demo (login → intake → generar plan → aprobar) en el Typeform o en el PR de entrega.
...
**Despliegue demo (Entrega 2):** `docs/deploy-entrega2-demo.md` + rama `feature-entrega2-AVW` (`render.yaml`).
...
Diagramas adicionales (C4, secuencia RAG, ER ampliado): `docs/10-solution-diagrams.md`, narrativa en `docs/02-system-architecture.md`.
...
├── docs/                 # fases académicas, sprints, piloto, deploy
├── prompts.md            # registro de prompts LIDR
├── README-ORIGINAL.md    # README de producto (inglés)
├── docker-compose.yml
└── README.md             # este documento (formato LIDR para Typeform)
```
...
**Demo cloud (Entrega 2):** Render Blueprint en rama `feature-entrega2-AVW` (`render.yaml`):
...
Análisis de opciones futuras: `holisticare_deployment_analysis.md`. Runbook: `docs/06-deployment-and-ops-runbook.md`.
...
- Auditoría y backlog de seguridad: `docs/09-security-audit-and-todos.md`.

### **2.6. Tests**

- **Backend:** pytest con `AsyncMock` de sesión DB y overrides de pipeline RAG (`conftest.py`). Cubren generación de planes, ingesta, intake, diario, plateau, predicción, memory bank, auth, seguridad SQL en chunks.
- **Frontend:** Vitest (utilidades, intake builder, errores API) + ESLint + build de producción.
- **CI:** `.github/workflows/ci.yml` — pytest + lint + test + build (+ audits de seguridad).
...
DDL completo: `infra/init.sql`. Diccionario y privacidad: `docs/03-data-dictionary-and-privacy-framework.md`.
...
Backlog completo: `docs/04-feature-specs-and-user-stories.md`.
...
Sprints: `docs/sprint-01.md` … `docs/sprint-10.md`.
